...
public class AllDeactivatedAcActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> deactivated_maplist = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.all_deactivated_ac);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		search_text.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (!(0 == map_temp.length())) {
					deactivated_maplist.clear();
					deactivated_maplist = new Gson().fromJson(map_temp, new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
					map_length = deactivated_maplist.size();
					search_pos = map_length - 1;
					for(int _repeat18 = 0; _repeat18 < (int)(map_length); _repeat18++) {
						value1 = deactivated_maplist.get((int)search_pos).get("name").toString();
						if (!(_charSeq.length() > value1.length()) && value1.toLowerCase().contains(_charSeq.toLowerCase())) {
							
						}else {
							deactivated_maplist.remove((int)(search_pos));
						}
						search_pos--;
					}
					listview1.setAdapter(new Listview1Adapter(deactivated_maplist));
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				msg = "";
				msg = "".concat("ID No.").concat(deactivated_maplist.get((int)_position).get("idno").toString().concat("\n"));
				msg = msg.concat("Name : ").concat(deactivated_maplist.get((int)_position).get("name").toString().concat("\n"));
				msg = msg.concat("Gender :").concat(deactivated_maplist.get((int)_position).get("gender").toString().concat("\n"));
				msg = msg.concat("Email : ").concat(deactivated_maplist.get((int)_position).get("email").toString().concat("\n"));
				msg = msg.concat("Contact No : ").concat(deactivated_maplist.get((int)_position).get("contactno").toString().concat("\n"));
				msg = msg.concat("Department : ").concat(deactivated_maplist.get((int)_position).get("department_name").toString().concat("\n"));
				if ("Male".equals(deactivated_maplist.get((int)_position).get("gender").toString())) {
					msg = msg.concat("He is a ").concat(deactivated_maplist.get((int)_position).get("who_are_you").toString().concat("\n"));
				}else {
					msg = msg.concat("She is a ").concat(deactivated_maplist.get((int)_position).get("who_are_you").toString().concat("\n"));
				}
				msg = msg.concat("What to do?");
				d.setTitle("User Profile");
				d.setMessage(msg);
				d.setPositiveButton("Active", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface _dialog, int _which) {
						d1.setCancelable(false);
						d1.setTitle("⚠️ Alert ⚠️");
						d1.setMessage("Are you sure ?");
						d1.setPositiveButton("Active", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface _dialog, int _which) {
								map.clear();
								map = deactivated_maplist.get((int)_position);
								map.remove("status");
								map.put("status", "active");
								allusers.child(deactivated_maplist.get((int)_position).get("idno").toString()).updateChildren(map);
								msg = "";
				...
				d.setNeutralButton("Delete", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface _dialog, int _which) {
						...
						d1.setPositiveButton("Delete", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface _dialog, int _which) {
								allusers.child(deactivated_maplist.get((int)_position).get("idno").toString()).removeValue();
								int1.setClass(getApplicationContext(), AllDeactivatedAcActivity.class);
								startActivity(int1);
								finish();
							}
		...
		_allusers_child_listener = new ChildEventListener() {
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				if (_childValue.get("status").toString().equals("deactivate")) {
					deactivated_maplist.add(_childValue);
				}
				map_temp = new Gson().toJson(deactivated_maplist);
				listview1.setAdapter(new Listview1Adapter(deactivated_maplist));
				((BaseAdapter)listview1.getAdapter()).notifyDataSetChanged();
			}
	...
	private void initializeLogic() {
		setTitle("All Deactivated Accounts List");
		t = new TimerTask() {
			@Override
			public void run() {
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			subject_name.setText(_data.get((int)_position).get("who_are_you").toString());
