...
public class AllTeachersAcActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> teacher_maplist = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.all_teachers_ac);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		search_text.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (!(0 == map_temp.length())) {
					teacher_maplist.clear();
					teacher_maplist = new Gson().fromJson(map_temp, new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
					map_length = teacher_maplist.size();
					search_pos = map_length - 1;
					for(int _repeat18 = 0; _repeat18 < (int)(map_length); _repeat18++) {
						value1 = teacher_maplist.get((int)search_pos).get("name").toString();
						if (!(_charSeq.length() > value1.length()) && value1.toLowerCase().contains(_charSeq.toLowerCase())) {
							
						}else {
							teacher_maplist.remove((int)(search_pos));
						}
						search_pos--;
					}
					listview1.setAdapter(new Listview1Adapter(teacher_maplist));
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				msg = "";
				msg = "".concat("ID No. ").concat(teacher_maplist.get((int)_position).get("idno").toString().concat("\n"));
				msg = msg.concat("Name : ").concat(teacher_maplist.get((int)_position).get("name").toString().concat("\n"));
				msg = msg.concat("Gender : ").concat(teacher_maplist.get((int)_position).get("gender").toString().concat("\n"));
				msg = msg.concat("Email : ").concat(teacher_maplist.get((int)_position).get("email").toString().concat("\n"));
				msg = msg.concat("Contact No : ").concat(teacher_maplist.get((int)_position).get("contactno").toString().concat("\n"));
				msg = msg.concat("Department : ").concat(teacher_maplist.get((int)_position).get("department_name").toString().concat("\n"));
				msg = msg.concat("Password : ").concat(teacher_maplist.get((int)_position).get("password").toString().concat("\n"));
				...
				d.setPositiveButton("Deactivate", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface _dialog, int _which) {
						...
						d1.setPositiveButton("Deactivate", new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface _dialog, int _which) {
								map.clear();
								map = teacher_maplist.get((int)_position);
								map.remove("status");
								map.put("status", "deactivate");
								allusers.child(teacher_maplist.get((int)_position).get("idno").toString()).updateChildren(map);
				...
				d.setNeutralButton("Delete", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface _dialog, int _which) {
						...
						d1.setPositiveButton("Delete", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface _dialog, int _which) {
								allusers.child(teacher_maplist.get((int)_position).get("idno").toString()).removeValue();
								int1.setClass(getApplicationContext(), AllTeachersAcActivity.class);
								startActivity(int1);
								finish();
							}
		...
		_allusers_child_listener = new ChildEventListener() {
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				if (_childValue.get("who_are_you").toString().equals("Teacher") && _childValue.get("status").toString().equals("active")) {
					teacher_maplist.add(_childValue);
				}
				map_temp = new Gson().toJson(teacher_maplist);
				listview1.setAdapter(new Listview1Adapter(teacher_maplist));
				((BaseAdapter)listview1.getAdapter()).notifyDataSetChanged();
			}
	...
	private void initializeLogic() {
		setTitle("All Teacher's Account");
		t = new TimerTask() {
			@Override
			public void run() {
